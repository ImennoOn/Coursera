...
```{r LoadingData, cache=TRUE, warning=FALSE, message=FALSE, echo=FALSE, results='hide'}
data(mtcars)
mtcars[1:3,]
...
```
Since p-Value is `r pVal` the automatic and manual transmissions are from different populations. And the mean for MPG of manual transmitted cars is about `r result$estimate[[2]] - result$estimate[[1]]` more than that of automatic transmitted cars.  

## Regression Model
Testing simple model.
```{r SimpleMpgModel, results="hide"}
...
The measure of how much an observation has effected the estimate of a regression coefficient, we get `r sum((abs(dfbetas(mpgModel)))>1)`
